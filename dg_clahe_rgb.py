...
def dual_gamma_clahe_rgb(
    ...
    unpad_indices = tuple(
        [
            # The original slice calculation was:
            # slice(pad_h, im - pad_w)
            # for pad_h, pad_w, im in zip(pad_start, pad_end, gray_image.shape)
            # This resulted in an incorrectly sized unpadded image.

            # Corrected slice calculation:
            # Slice from p_start_axis for a length of original_dim_axis
            slice(p_start_axis, p_start_axis + original_dim_axis)
            for p_start_axis, original_dim_axis in zip(pad_start, gray_image.shape) # gray_image.shape here refers to its dimensions before padding
        ]
    )
